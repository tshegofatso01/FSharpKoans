...
module ``07: The Good Kind of Discrimination`` = 
    ...
    let ``02 Creating & pattern-matching a discriminated union`` () = 
        ...
        randomOpinion (BSc(Management, Economics)) |> should equal "!!SCIENCE!!"
    ...
    let ``06 A discriminated union can refer to itself (i.e., it can be recursive).`` () =
        ...
        let a = Node("a", Empty, Node("a", Empty, Node("a", Empty, Node("a",Empty, Empty)))) // <-- you may want to spread this over multiple lines and/or let-bindings ...!
